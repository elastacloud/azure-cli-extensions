...
class LoadBalancerBuildingBlock(BuildingBlock):
    ...
    def get_ip_addresses(self, load_balancers):
        public_ip_addresses = []

        for load_balancer in load_balancers:
            ip_addresses = [load_balancer.frontend_ip_configurations for load_balancer in self.settings if load_balancer.frontend_ip_configurations]
            ip_addresses_public = [ip_address for ip_address in ip_addresses[0] if ip_address.load_balancer_type == 'Public']
            ...
            for ip_address in ip_addresses_public:
                public_ip_address_parameters = {
                    ...
                    'name': '{}-{}-pip'.format(load_balancer.name, ip_address.name),
        ...
        if self.load_balancer_type == 'Internal':
            ...
            model = factory(
                ...
                public_ip_address = SubResource(id=resource_id(
                    ...
                    name='{}-{}-pip'.format(self.load_balancer.name, self.name))) # pylint: disable=E1101
            )
