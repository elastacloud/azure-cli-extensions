...
class NetworkInterface(TaggedResource, TopLevelResource, Resource):
    ...
    def transform(self):
        ...
        model = factory(
            ...
            name=self.name if self.name else '{}-nic'.format(self.virtual_machine.name),
            ...
            #    public_ip_allocation_method = self.public_ip_allocation_method, subnet_name = self.subnet_name, virtual_network_name = self.name if self.name else '{}-nic'.format(self.virtual_machine.name)),
